...
export async function receiveProcurementOrder(store: ProcurementStore, input: {
  ...
  for (const update of lineUpdates) {
    const receiptLineId = `procurement_receipt_line_${crypto.randomUUID()}`;
    ...
    if (update.line.inventoryItemId) {
      await store.increaseInventory({
        inventoryItemId: update.line.inventoryItemId,
        quantity: update.receiptLine.receivedQuantity,
        referenceId: receiptLineId,
        actorUserId: input.actorUserId,
        lotNumber: update.receiptLine.lotNumber ?? null,
        location: update.receiptLine.location ?? null,
      });
    }
    await store.updateLineReceivedQuantity({ lineId: update.line.id, quantityReceived: update.newReceived });
  }

  const status: ProcurementOrderStatus = isFinal ? "completed" : "partially_received";
